...
public class TweakConfig {
    ...
    @Config.Comment({
            "Enabling this will show an atlas button in waystone selection screens (when rightclicking a waystone or using a warp scroll).",
            "Clicking on the button will display all available waystones in the current dimension on the current held atlas (requires having an atlas). " +
            "Clicking on a waystone marker will tp to it."
    })
    @Config.Name("Select Waystone on map")
    @Config.RequiresMcRestart
    ...
    public boolean selectWaystoneOnMap = true;
}
